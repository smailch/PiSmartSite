...
import { SplitSquareHorizontal, ChevronLeft, ChevronRight, MoveHorizontal } from 'lucide-react';
  ...
  return (
    ...
    >
      ...
      <img
        ...
        draggable={false}
        onError={(e) => { e.currentTarget.src = 'https://placehold.co/800x450/1e293b/475569?text=Photo+Not+Available'; }}
      />
    ...
    >
      <div className="relative aspect-video bg-slate-900/60">
        <img
          ...
          onError={(e) => { e.currentTarget.src = 'https://placehold.co/400x225/1e293b/475569?text=No+Photo'; }}
        />
        {selected && (
          <div
...
export default function ComparePage() {
  ...
  useEffect(() => {
    fetch(`${API_BASE}/progress-photos`)
      .then((r) => r.json())
      .then((data: ProgressPhoto[]) => {
        // Sort by takenAt ascending so "before" is naturally earlier
  ...
  // Unique project IDs that have at least 2 photos
  ...
  // Auto-pick before/after when project changes
  useEffect(() => {
  ...
  const beforeUrl = beforePhoto
    ? `${API_BASE}${beforePhoto.photoUrl.startsWith('/') ? '' : '/'}${beforePhoto.photoUrl}`
    : null;
  const afterUrl = afterPhoto
    ? `${API_BASE}${afterPhoto.photoUrl.startsWith('/') ? '' : '/'}${afterPhoto.photoUrl}`
    : null;
    ...
    <MainLayout>
      ...
      {loading ? (
        ...
        <div className="rounded-xl border border-white/10 bg-card/60 py-16 text-center backdrop-blur-sm">
          ...
          <p className="mt-1 text-xs text-slate-500">Upload at least 2 photos to the same project to compare them.</p>
        </div>
      ) : (
            ...
            <div>
              ...
              <div className="mt-4 grid grid-cols-2 gap-4">
                ...
                <div className="rounded-xl border border-blue-400/20 bg-blue-500/5 p-4">
                  ...
                  <p className="mt-1 text-xs text-slate-500">
                    {beforePhoto && new Date(beforePhoto.takenAt).toLocaleDateString('en-US', { dateStyle: 'medium' })}
                  </p>
                ...
                <div className="rounded-xl border border-emerald-400/20 bg-emerald-500/5 p-4">
                  ...
                  <p className="mt-1 text-xs text-slate-500">
                    {afterPhoto && new Date(afterPhoto.takenAt).toLocaleDateString('en-US', { dateStyle: 'medium' })}
                  </p>
              ...
              {progressDelta !== null && (
                <div className="mt-4 flex justify-center">
                  <span className={`rounded-full px-5 py-2 text-sm font-semibold ring-1 ${
                    progressDelta >= 0
          ...
          <div>
            <div className="mb-4 flex items-center gap-3">
              <h3 className="text-sm font-semibold text-slate-300">
                Pick photos — {projectPhotos.length} photo{projectPhotos.length !== 1 ? 's' : ''} in this project
              </h3>
            ...
            <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-4">
              {projectPhotos.map((photo) => (
                <PhotoPickerCard
                  ...
                  onClick={() => picking ? handlePickPhoto(photo) : undefined}
                />
              ))}
            </div>
          </div>
