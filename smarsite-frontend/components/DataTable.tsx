          ...
          data.map((row, idx) => (
            ...
            >
              {columns.map((col) => (
                <td
                  key={String(col.key)}
                  className={`px-4 py-3.5 text-foreground first:pl-5 last:pr-5 sm:px-5 ${pageLevelScroll ? "align-top break-words" : "max-w-[min(28rem,50vw)] align-middle"} ${cellAlignClass(col.align)}`}
                >
