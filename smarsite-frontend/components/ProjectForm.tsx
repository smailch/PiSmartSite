...
import type { Project, ProjectType, Human, ClientAccount } from "@/lib/types";
...
interface ProjectFormProps {
  ...
  siteEngineers?: Human[];
  /** Comptes rôle « Client » (chargés par la page appelante, JWT). */
...
function getClientLabel(a: ClientAccount): string {
  return [a.fullName, a.email].filter(Boolean).join(" · ");
}
    ...
    <Popover open={open} onOpenChange={setOpen}>
      ...
      >
        <Command className="rounded-xl">
          <CommandInput placeholder="Search by name or email…" className="h-10" />
              ...
              {clients.map((c) => {
                ...
                return (
                  <CommandItem
                    ...
                    onSelect={() => {
                      onChange(isSelected ? "" : c._id);
                      setOpen(false);
                    }}
                    className="flex items-center gap-3 rounded-lg px-2.5 py-2.5 cursor-pointer"
                  >
                    <span
                      className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-full text-xs font-bold ring-1 ${colorForId(c._id)}`}
                    >
                      {(c.fullName || "?")
                        .split(/\s+/)
                        .map((p) => p[0])
...
export default function ProjectForm({
  ...
  const validate = useCallback((): ProjectFieldErrors => {
    ...
    if (!clientId.trim()) {
      next.clientId = "Please select a client account.";
    }

    return next;
  }, [name, description, startDate, endDate, budget, location, createdBy, clientId]);
  ...
  useEffect(() => {
    if (initialData) {
      ...
      setClientId(
        initialData.clientId && typeof initialData.clientId === "string"
          ? initialData.clientId
          : String(initialData.clientId ?? "").trim(),
      );
      setErrors({});
    } else {
      setName("");
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (clientId.trim()) {
      (payload as { clientId?: string }).clientId = clientId.trim();
    }
    ...
    <form noValidate onSubmit={handleSubmit} className="flex flex-col gap-5">
      <div className="rounded-2xl border border-border/80 bg-card p-4 shadow-sm sm:p-5">
        <div className="grid grid-cols-1 gap-x-5 gap-y-4 md:grid-cols-2">
          ...
          <div className={groupClass("clientId")}>
            <label htmlFor="project-clientId-combobox" className={fieldLabelClass}>
              Client account
            </label>
            <ClientCombobox
            ...
            {clientAccounts.length === 0 && (
              <p className="text-xs text-amber-600 dark:text-amber-400" role="status">
                No client accounts loaded. Sign in with a non-client user and check the API
