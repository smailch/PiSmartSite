...
import { fetcher, getApiBaseUrl, getAuthHeaderInit, getProjectsKey } from '@/lib/api';
      ...
      >
        ...
        <g transform={`translate(${padL},${padT})`}>
          ...
          {points.map((p, i) => {
            ...
            const statusColor =
              p.status === 'approved' ? '#34d399' : p.status === 'rejected' ? '#f87171' : '#fbbf24';
            return (
              <g key={i}>
                <rect
                  x={cx - 18}
                  ...
                  height={H}
                  fill="transparent"
                  onMouseEnter={() => setTooltip({ x: cx + padL, y: cy + padT, point: p })}
                />
...
export default function ProgressTrendPage() {
  ...
  const chartDataByProject = useMemo(() => {
    const map = new Map<string, { date: string; progress: number; caption?: string; status: string }[]>();
          ...
          {projectsToShow.map((projectId) => {
            ...
            return (
              ...
              >
                ...
                <div className="flex flex-wrap items-center gap-4 border-t border-white/[0.06] px-6 py-3 text-xs text-slate-500">
                  <span className="flex items-center gap-1.5">
                    <span className="inline-block h-2 w-2 rounded-full bg-emerald-400" /> Approved
                  </span>
                  <span className="flex items-center gap-1.5">
                    <span className="inline-block h-2 w-2 rounded-full bg-amber-400" /> Pending
                  </span>
                  <span className="flex items-center gap-1.5">
                    <span className="inline-block h-2 w-2 rounded-full bg-red-400" /> Rejected
                  </span>
