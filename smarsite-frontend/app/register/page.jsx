...
const Register = () => {
  ...
  useEffect(() => {
    ...
    const allowed = new Set(REGISTER_ROLE_ORDER);
    axios
      .get(`${base}/roles`)
      .then((res) => {
        const list = Array.isArray(res.data) ? res.data : [];
        /* Inscription publique : uniquement Client, Project Manager, Site Engineer, Financier. */
        const registerRoles = sortRegisterRoles(
          list.filter(
            (r) => r?.name && allowed.has(normalizeRoleName(r.name)),
          ),
        );
        setRoles(registerRoles);
      })
