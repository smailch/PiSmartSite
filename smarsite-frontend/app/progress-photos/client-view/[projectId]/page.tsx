...
export default function ClientViewPage() {
  ...
  const fetchApprovedPhotos = async () => {
    try {
      const response = await fetch(`http://localhost:3000/progress-photos/project/${projectId}/approved`);
  ...
  const fetchLatestProgress = async () => {
    try {
      const response = await fetch(`http://localhost:3000/progress-photos/project/${projectId}/latest-progress`);
      const data = await response.json();
