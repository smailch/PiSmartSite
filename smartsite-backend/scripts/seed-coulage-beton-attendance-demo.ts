/**
 * Démo : job « Coulage du béton », 5 travailleurs, pointages sur un mois (jours ouvrables uniquement).
 * Même scénario que seed-peinture-attendance-demo.ts (profils de présence / absences).
 *
 * Usage (depuis smartsite-backend/) :
 *   npm run seed:coulage-beton
 *
 * Variables optionnelles :
 *   MONGODB_URI   — URI Mongo (obligatoire en prod ; sinon valeur du script Peinture)
 *   SEED_YEAR     — ex. 2026
...
const JOB_TITLE = 'Coulage du béton';
...
const TUNISIAN_DEMO_WORKERS: ReadonlyArray<{
  ...
  { firstName: 'Slim', lastName: 'Gharbi', phoneSuffix: '56111234' },
  { firstName: 'Nour', lastName: 'Hadded', phoneSuffix: '56222345' },
  { firstName: 'Karim', lastName: 'Sassi', phoneSuffix: '56333456' },
  { firstName: 'Leïla', lastName: 'Chakroun', phoneSuffix: '56444567' },
  { firstName: 'Hichem', lastName: 'Dhahri', phoneSuffix: '56555678' },
];
...
async function main(): Promise<void> {
  ...
  if (!project || !project._id) {
    const ins = await col.projects.insertOne({
      name: 'Chantier démo — Coulage du béton',
  ...
  if (!task || !task._id) {
    const tins = await col.tasks.insertOne({
      title: 'Tâche démo — support job Coulage du béton',
  ...
  if (!job) {
    const ins = await col.jobs.insertOne({
      taskId,
      title: JOB_TITLE,
      description: 'Démo présence — coulage béton (données de test)',
      startTime,
      endTime,
      status: 'En cours',
      assignedResources,
  ...
  if (!job || !job._id) {
    throw new Error(`Job « ${JOB_TITLE} » introuvable après insertion`);
  }
  ...
  console.log('');
  console.log(
    `Ouvrez le job « ${JOB_TITLE} » dans l’app → Pointage, choisissez le mois, puis « Analyser ».`,
  );
}
